...
export default async function handler(
  ...
  try {
    const { oldNftTransactionId, userAccountId, serialNumber } = req.body;

    if (!oldNftTransactionId || !userAccountId || !serialNumber) {
      return res.status(400).json({
        error: 'Missing required fields: oldNftTransactionId, userAccountId, serialNumber'
      });
    }

    console.log(`Processing swap for user ${userAccountId}, NFT #${serialNumber}`);
    ...
    // Verify the NFT was actually transferred to blackhole
    // (Additional verification could be done here by querying mirror node)

    // Now send new NFT from treasury to user
    console.log('Sending new NFT to user...');

    const newNftTransaction = new TransferTransaction()
      .addNftTransfer(
        TokenId.fromString(NEW_TOKEN_ID),
        serialNumber, // Same serial number
        AccountId.fromString(TREASURY_ACCOUNT_ID),
        AccountId.fromString(userAccountId)
      )
      .setTransactionMemo(`Swap completed for Degen Ape #${serialNumber}`)
      .freezeWith(client);
    ...
    console.log('New NFT transferred successfully:', newNftTransactionId);
